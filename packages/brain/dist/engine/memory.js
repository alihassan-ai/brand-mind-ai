...
export async function generateBusinessMemory(shopId) {
    ...
    // 1. Identity Memory - Price Tier & Catalog
    const allPrices = shop.products.flatMap((p) => p.variants.map((v) => Number(v.price)));
    const medianPrice = calculateMedian(allPrices);
    const productCount = shop.products.length;
    const productTypes = Array.from(new Set(shop.products.map((p) => p.productType).filter(Boolean)));
    // 2. Discount Dependence Analysis
    const discountedOrders = shop.orders.filter((o) => Number(o.totalDiscounts) > 0);
    ...
    const dailyRevenues = shop.dailyMetrics.map((m) => Number(m.netRevenue));
    ...
    const averageAOV = shop.dailyMetrics.length > 0
        ? shop.dailyMetrics.reduce((acc, m) => acc + Number(m.aov), 0) / shop.dailyMetrics.length
        : 0;
...
function calculateAverageOrderInterval(orders) {
    if (orders.length < 2)
