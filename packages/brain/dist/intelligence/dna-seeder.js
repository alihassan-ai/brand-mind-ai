...
// ============================================
export async function seedDNAFromShopify(shopId) {
    ...
    if (existingDna) {
        ...
        await prisma.storeDNA.create({
            data: Object.assign({ shopId }, dnaData),
        });
    }
...
async function gatherShopifyMetrics(shopId) {
    ...
    const topProducts = lineItemsWithRevenue
        ...
        return {
            ...
            productType: (product === null || product === void 0 ? void 0 : product.productType) || undefined,
        };
    })
    ...
    `;
        repeatCustomerCount = Number(((_a = repeatCustomers[0]) === null || _a === void 0 ? void 0 : _a.count) || 0);
    }
    catch (e) {
...
function calculateHeroProductShare(metrics) {
    if (metrics.topProducts.length === 0 || metrics.totalRevenue === 0)
        return 0;
...
function calculateVelocityTrend(metrics) {
    if (metrics.monthlyRevenue.length < 2)
        return 'stable';
...
function calculateRevenueGrowth(metrics) {
    ...
    if (metrics.monthlyRevenue.length < 2)
        return 0;
    const lastMonth = ((_a = metrics.monthlyRevenue[metrics.monthlyRevenue.length - 1]) === null || _a === void 0 ? void 0 : _a.revenue) || 0;
    const prevMonth = ((_b = metrics.monthlyRevenue[metrics.monthlyRevenue.length - 2]) === null || _b === void 0 ? void 0 : _b.revenue) || 0;
    if (prevMonth === 0)
        return 0;
...
function determineCategoryDepth(metrics) {
    ...
    if (avgProductsPerType < 5)
        return 'shallow';
    if (avgProductsPerType < 15)
        return 'moderate';
    return 'deep';
}
...
function inferCashPosition(bcg) {
    ...
    if (hasCashCows && hasStars)
        return 'cash_rich';
    if (manyDogs && !hasCashCows)
        return 'tight';
    return 'balanced';
}
...
// ============================================
export async function seedDNAFromDomainAnalysis(shopId, analysis) {
    const existingDna = await prisma.storeDNA.findUnique({ where: { shopId } });
    const dnaData = {
